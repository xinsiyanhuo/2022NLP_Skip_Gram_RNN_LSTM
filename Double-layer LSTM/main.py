...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        # 第二层
        self.W2_c1 = nn.Linear(emb_size, n_hidden, bias=False)  # 开始输入门的参数
        self.W2_c2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W2_i1 = nn.Linear(emb_size, n_hidden, bias=False)  # 输入阶段参数
        self.W2_i2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W2_f1 = nn.Linear(emb_size, n_hidden, bias=False)  # 忘记阶段参数
        self.W2_f2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W2_o1 = nn.Linear(emb_size, n_hidden, bias=False)  # 输出阶段参数
        self.W2_o2 = nn.Linear(n_hidden, n_hidden, bias=False)
        ...
        # 第二层
        self.b2_i = nn.Parameter(torch.ones([n_hidden]))  # 输入阶段
        self.b2_f = nn.Parameter(torch.ones([n_hidden]))  # 忘记阶段
        self.b2_c = nn.Parameter(torch.ones([n_hidden]))  # 记忆阶段
        self.b2_o = nn.Parameter(torch.ones([n_hidden]))  # 输出阶段
    ...
    def forward(self, X):
        ...
        h2_t = torch.zeros(sample_size, n_hidden)
        c2_t = torch.zeros(sample_size, n_hidden)
        ...
        for x2 in X2:
            f_t = torch.sigmoid(self.W2_f1(x2) + self.W2_f2(h2_t) + self.b2_f)  # 忘记阶段
            i_t = torch.sigmoid(self.W2_i1(x2) + self.W2_i2(h2_t) + self.b2_i)  # 输入阶段
            g_t = self.tanh(self.W2_c1(x2) + self.W2_c2(h2_t) + self.b2_c)
            c2_t = torch.mul(c2_t, f_t) + torch.mul(i_t, g_t)  # 记忆阶段
            o_t = torch.sigmoid(self.W2_o1(x2) + self.W2_o2(h2_t) + self.b2_o)  # 输出阶段
            h2_t = torch.mul(o_t, self.tanh(c2_t))

        # 输出yt y^(t) = softmx(Wa ^ (t) + b)
        model_output = self.W(h2_t) + self.b
        '''end'''
        return model_output
